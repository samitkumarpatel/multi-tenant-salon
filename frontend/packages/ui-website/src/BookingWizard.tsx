...
function SelectionSummary({
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-1.5 mb-6">
      {chips.map((c) => (
        <span key={c} className="text-[11px] font-semibold px-2.5 py-1 rounded-full border"
    ...
    <div>
      ...
      {services.length === 0 ? (
        ...
        <>
          ...
          {filtered.length === 0 ? (
            <p className="text-sm text-slate-400 text-center py-10">No services match "{query}".</p>
          ) : (
      ...
      <button onClick={onNext} disabled={!selected} className={`w-full py-3 ${primaryBtnCls}`} style={primaryBtnStyle(accent, !!selected)}>
...
function defaultBookingDate(hours: OperatingHours[]): string {
  ...
  for (let i = 0; i < 14; i++) {
    ...
    if (!h) { if (hours.length === 0) return toISODate(d); continue; }
...
function WeekGrid({
  ...
  saloonId: string; serviceId: number; staffId: number | null; date: string;
  setDate: (d: string) => void; selectedSlot: AvailableSlot | null; accent: Accent;
  closedDays: Set<string>; onPick: (date: string, slot: AvailableSlot) => void;
}) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-3">
      <div className="flex items-center justify-between mb-3">
        <button type="button" onClick={() => setWeekStart((w) => { const p = new Date(w); p.setDate(p.getDate() - 7); return p; })}
          disabled={!canPrev} className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-500 hover:bg-slate-100 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer transition-colors" aria-label="Previous week">
          <ArrowLeft className="w-3.5 h-3.5" />
        </button>
        <span className="text-sm font-bold text-slate-900">{rangeLabel}</span>
        <button type="button" onClick={() => setWeekStart((w) => { const n = new Date(w); n.setDate(n.getDate() + 7); return n; })}
          className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-500 hover:bg-slate-100 cursor-pointer transition-colors" aria-label="Next week">
        ...
        {days.map((d) => {
          ...
          return (
            <div key={iso} className="min-w-0">
              <button type="button" disabled={!clickable} onClick={() => setDate(iso)}
                ...
                style={isSelDay ? { backgroundColor: accent.tint, boxShadow: `inset 0 0 0 1.5px ${accent.color}` } : isToday ? { backgroundColor: accent.tint } : {}}>
                <span className="text-[9px] font-bold uppercase tracking-wider text-slate-400">{d.toLocaleDateString(undefined, { weekday: "short" }).slice(0, 2)}</span>
                <span className="text-sm font-bold" style={{ color: isSelDay || isToday ? accent.color : isPast || isClosed ? "#cbd5e1" : "#0f172a" }}>{d.getDate()}</span>
      ...
      {loading && (
        <div className="flex items-center justify-center py-6">
          <div className="w-6 h-6 border-2 rounded-full animate-spin" style={{ borderColor: accent.tint, borderTopColor: accent.color }} />
        </div>
      )}
      ...
      {!loading && !error && date && slotsByDate[date] !== undefined && (
        <div className="mt-3 pt-3 border-t border-slate-100" key={date}>
          <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Times · {fmtDate(date)}</p>
          {(slotsByDate[date] ?? []).length === 0 ? (
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
              {[...new Map((slotsByDate[date] ?? []).map((s) => [s.startTime, s])).values()].map((s) => {
                ...
                return (
                  <button key={s.startTime} type="button" onClick={() => onPick(date, s)}
                    className="py-2 rounded-lg text-xs font-semibold transition-all cursor-pointer border hover:scale-[1.04]"
                    style={isSel ? { backgroundColor: accent.color, color: accent.text, borderColor: accent.color } : { backgroundColor: accent.tint, color: accent.color, borderColor: "transparent" }}>
                    {fmt12(s.startTime)}
                  </button>
                );
              })}
...
function MonthCalendar({ date, setDate, accent, closedDays }: {
  date: string; setDate: (d: string) => void; accent: Accent; closedDays: Set<string>;
}) {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-3">
      <div className="flex items-center justify-between mb-2">
        <button type="button" onClick={() => setMonth((m) => new Date(m.getFullYear(), m.getMonth() - 1, 1))}
          disabled={!canPrev} className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-500 hover:bg-slate-100 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer transition-colors" aria-label="Previous month">
          <ArrowLeft className="w-3.5 h-3.5" />
        </button>
        <span className="text-sm font-bold text-slate-900">{month.toLocaleDateString(undefined, { month: "long", year: "numeric" })}</span>
        <button type="button" onClick={() => setMonth((m) => new Date(m.getFullYear(), m.getMonth() + 1, 1))}
          className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-500 hover:bg-slate-100 cursor-pointer transition-colors" aria-label="Next month">
          <ArrowRight className="w-3.5 h-3.5" />
        </button>
      </div>
      ...
      <div className="grid grid-cols-7 gap-y-0.5">
        ...
        {Array.from({ length: daysInMonth }, (_, i) => {
          ...
          const isToday = d.getTime() === today.getTime();
          return (
            <button key={iso} type="button" disabled={disabled} onClick={() => setDate(iso)}
              className={`h-8 w-8 mx-auto rounded-full text-xs font-semibold transition-colors flex items-center justify-center ${disabled ? "text-slate-300 cursor-not-allowed" : "cursor-pointer hover:bg-slate-100 text-slate-700"}`}
              style={isSel ? { backgroundColor: accent.color, color: accent.text } : isToday ? { boxShadow: `inset 0 0 0 1.5px ${accent.color}`, color: accent.color } : {}}>
              {i + 1}
            </button>
          );
        })}
...
function DesignerGrid({
  ...
  saloonId: string; serviceId: number; staff: StaffMember[]; date: string;
  selectedSlot: AvailableSlot | null; accent: Accent; closedDays: Set<string>;
  onPick: (date: string, slot: AvailableSlot) => void;
}) {
  const [slots, setSlots] = useState<AvailableSlot[] | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const isClosed = date ? closedDays.has(JS_DAY_NAME[new Date(`${date}T00:00:00`).getDay()]) : false;
      ...
      {!loading && !error && date && !isClosed && slots !== null && (
        times.length === 0 ? (
          ...
          <div className="rounded-xl border border-slate-200 overflow-hidden">
            <div className="grid bg-slate-50/70 border-b border-slate-200"
              ...
              {staff.map((m) => (
                <div key={m.id} className="flex flex-col items-center gap-1 py-2.5 px-1 border-l border-slate-200 min-w-0">
                  <div className="w-9 h-9 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0 overflow-hidden"
                    style={{ backgroundColor: accent.tint, color: accent.color }}>
                    {(m.photoUrls?.[0] ?? m.photoUrl) ? (
                      <img src={m.photoUrls?.[0] ?? m.photoUrl} alt={m.name} className="w-full h-full object-cover" loading="lazy"
            ...
            {times.map((t, ri) => (
              <div key={t} className={`grid ${ri > 0 ? "border-t border-slate-100" : ""}`}
                style={{ gridTemplateColumns: `3.25rem repeat(${staff.length}, minmax(0, 1fr))` }}>
                ...
                {staff.map((m) => {
                  ...
                  if (!slot) return (
                    <div key={m.id} className="h-10 border-l border-slate-100 p-0.5">
                      <button type="button" disabled className="w-full h-full rounded-md text-[10px] font-semibold cursor-not-allowed flex items-center justify-center bg-slate-100 text-slate-300">Unavailable</button>
                    </div>
                  );
                  const isSel = selectedSlot?.staffId === slot.staffId && selectedSlot?.startTime === slot.startTime;
                  return (
                    <div key={m.id} className="h-10 border-l border-slate-100 p-0.5">
                      <button type="button" onClick={() => onPick(date, slot)}
                        ...
                        style={isSel ? { backgroundColor: accent.color, color: accent.text } : { backgroundColor: accent.tint, color: accent.color, boxShadow: `inset 2px 0 0 ${accent.color}` }}>
...
function DaySlots({
  ...
  saloonId: string; serviceId: number; date: string; staffId: number | null; staff: StaffMember[];
  selectedSlot: AvailableSlot | null; accent: Accent; closedDays: Set<string>;
  onPick: (date: string, slot: AvailableSlot) => void;
}) {
  const [slots, setSlots] = useState<AvailableSlot[] | null>(null);
  ...
  const groups: [string, AvailableSlot[]][] = slots
    ? ([
      ...
      ] as [string, AvailableSlot[]][]).filter(([, list]) => list.length > 0)
    : [];
...
function StepDate({
  ...
  saloonId: string; serviceId: number; staff: StaffMember[]; date: string;
  setDate: (d: string) => void; staffId: number | null; setStaffId: (id: number | null) => void;
  selectedSlot: AvailableSlot | null; accent: Accent; closedDays: Set<string>;
  defaultMode?: "input" | "week" | "designer"; onBack: () => void; onNext: () => void;
  ...
  const staffPicker = staff.length > 0 && (
    <div>
      ...
      <div className="grid sm:grid-cols-2 gap-2">
        <button onClick={() => setStaffId(null)} className="w-full text-left px-4 py-3 rounded-xl border transition-all cursor-pointer flex items-center gap-3 bg-white hover:border-slate-300 hover:shadow-sm"
          ...
          <button key={s.id} onClick={() => setStaffId(s.id)}
            ...
            <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 overflow-hidden"
              ...
              {(s.photoUrls?.[0] ?? s.photoUrl) ? (
                <img src={s.photoUrls?.[0] ?? s.photoUrl} alt={s.name} className="w-full h-full object-cover" loading="lazy"
                  onError={(e) => { e.currentTarget.style.display = "none"; }} />
              ) : initials(s.name)}
            </div>
            <div>
  ...
  return (
    <div>
      ...
      <p className="text-sm text-slate-500 mb-5">
        {dateMode === "week" ? "Pick an available time straight from the week overview."
          : dateMode === "designer" ? "Pick a date, then choose a time with the stylist you prefer."
      ...
      <div className="space-y-4 mb-6">
        <div>
            ...
            <div className="flex items-center gap-0.5 bg-slate-100 rounded-lg p-0.5">
              {(["designer", "week", "input"] as const).map((m) => (
                <button key={m} type="button" onClick={() => setDateMode(m)}
                  ...
                  style={dateMode === m ? { backgroundColor: "#ffffff", color: accent.color, boxShadow: "0 1px 2px rgba(0,0,0,0.06)" } : { color: "#64748b" }}>
                  {MODE_LABEL[m]}
                </button>
              ))}
            </div>
          ...
          {dateMode === "week" ? (
            <WeekGrid saloonId={saloonId} serviceId={serviceId} staffId={staffId} date={date} setDate={setDate}
              selectedSlot={selectedSlot} accent={accent} closedDays={closedDays} onPick={onPickSlot} />
          ) : dateMode === "designer" ? (
            <div className="grid lg:grid-cols-[17rem_1fr] gap-3 items-start">
              <MonthCalendar date={date} setDate={setDate} accent={accent} closedDays={closedDays} />
              <DesignerGrid saloonId={saloonId} serviceId={serviceId} staff={staff} date={date}
                selectedSlot={selectedSlot} accent={accent} closedDays={closedDays} onPick={onPickSlot} />
            </div>
          ) : (
            <div className="space-y-3">
              ...
              <DaySlots saloonId={saloonId} serviceId={serviceId} date={date} staffId={staffId} staff={staff}
                selectedSlot={selectedSlot} accent={accent} closedDays={closedDays} onPick={onPickSlot} />
            </div>
          )}
        </div>
      ...
      <div className="flex gap-3">
        <button onClick={onBack} className={backBtnCls}><ArrowLeft className="w-4 h-4" /> Back</button>
...
function StepSlots({
  ...
  saloonId: string; serviceId: number; date: string; staffId: number | null;
  staffMap: Map<number, StaffMember>; selectedSlot: AvailableSlot | null; accent: Accent;
    ...
    <div>
      ...
      {loading && (
        <div className="flex items-center justify-center py-12">
          <div className="w-8 h-8 border-2 rounded-full animate-spin" style={{ borderColor: accent.tint, borderTopColor: accent.color }} />
        </div>
      )}
              ...
              <div key={label}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {list.map((s, i) => {
                    ...
                    return (
                      <button key={`${s.staffId}-${s.startTime}-${i}`} onClick={() => onSelect(s)}
      ...
      <div className="flex gap-3 mt-4">
        <button onClick={onBack} className={backBtnCls}><ArrowLeft className="w-4 h-4" /> Back</button>
        <button onClick={onNext} disabled={!selectedSlot} className={`flex-1 py-3 ${primaryBtnCls}`} style={primaryBtnStyle(accent, !!selectedSlot)}>
...
function StepDetails({
  ...
  accent: Accent; onBack: () => void; onSubmit: () => void; busy: boolean;
}) {
    ...
    <div>
      ...
      <div className="space-y-4 mb-6">
        <div>
          ...
          <input className={inputCls} style={ringStyle} placeholder="Jane Smith" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1.5">Email address <span className="text-red-500">*</span></label>
          <input type="email" className={inputCls} style={ringStyle} placeholder="jane@example.com" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
        </div>
        <div>
          ...
          <input type="tel" className={inputCls} style={ringStyle} placeholder="+1 555 000 0000" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} />
        </div>
        <div>
          ...
          <textarea className={`${inputCls} resize-none`} style={ringStyle} rows={2} placeholder="Anything we should know?" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
        </div>
      </div>
      <div className="flex gap-3">
        <button onClick={onBack} className={backBtnCls}><ArrowLeft className="w-4 h-4" /> Back</button>
        <button onClick={onSubmit} disabled={!valid || busy} className={`flex-1 py-3 ${primaryBtnCls}`} style={primaryBtnStyle(accent, !!valid && !busy)}>
          {busy ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Booking…</> : <>Confirm booking <Check className="w-4 h-4" /></>}
        </button>
      </div>
    </div>
  );
...
function StepConfirm({
  ...
  booking: Booking; service: ServiceItem; staff: StaffMember | undefined; saloon: Saloon; accent: Accent; onExit: () => void;
}) {
  return (
    <div className="text-center">
      ...
      <p className="text-sm text-slate-500 mb-6">A confirmation has been sent to <strong>{booking.customerEmail}</strong>.</p>
      <div className="rounded-2xl border border-slate-200 overflow-hidden mb-8 text-left shadow-sm">
        <div className="px-4 py-3 flex items-center justify-between" style={{ backgroundColor: accent.tint }}>
          <span className="text-xs font-bold uppercase tracking-wider" style={{ color: accent.color }}>Appointment</span>
          <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-white/70" style={{ color: accent.color }}>#{booking.id} · Pending</span>
        </div>
      ...
      <button onClick={onExit} className={`inline-flex px-6 py-3 ${primaryBtnCls}`} style={primaryBtnStyle(accent)}>
...
export function BookingWizard({
  ...
  const [date, setDate] = useState(() => defaultBookingDate(saloon.operatingHours ?? []));
    ...
    try {
      const booking = await apiFetch<Booking>(`${API}/${saloon.id}/bookings`, {
        ...
        body: JSON.stringify({
          ...
          customerEmail: form.email, customerPhone: form.phone || null,
          appointmentDate: date, startTime: slot.startTime, notes: form.notes || null,
        }),
      });
      setConfirmed(booking); setStep(5);
    } catch (e) {
      const msg = e instanceof Error ? e.message : "Booking failed";
      setError(/409|no longer available/i.test(msg) ? "Sorry, that slot was just taken. Please pick another time." : msg);
      if (/409|no longer available/i.test(msg)) { setSlot(null); setStep(viaWeek ? 2 : 3); }
    ...
    <div className="min-h-[100dvh] bg-slate-50 flex flex-col" style={{ fontFamily: fontStack }}>
      ...
      <main className="flex-1 flex items-start justify-center px-6 py-8"
        ...
        <div className={`w-full transition-all duration-300 ${step === 2 ? "max-w-5xl" : "max-w-lg"}`}>
          ...
          {step < 5 && (
            <SelectionSummary
              service={service} date={step > 2 ? date : ""} slot={step > 3 ? slot : null}
              staffName={step > 3 && slot ? staffMap.get(slot.staffId)?.name : undefined} accent={accent} />
          )}
          {error && (
          ...
          <div key={step} className={step < 5 ? "bg-white rounded-2xl border border-slate-200 shadow-sm p-5 sm:p-6" : ""}>
            {step === 1 && (
              <StepService services={services} selected={service} accent={accent}
                onSelect={(s) => { setService(s); setSlot(null); }} onNext={() => setStep(2)} />
            )}
            {step === 2 && service && (
              <StepDate saloonId={String(saloon.id)} serviceId={service.id} staff={staff} date={date}
                setDate={(d) => { setDate(d); setSlot(null); setViaWeek(false); }}
                staffId={staffId} setStaffId={(id) => { setStaffId(id); setSlot(null); }}
                selectedSlot={slot} accent={accent} closedDays={closedDays}
                defaultMode={preStaff ? "week" : "designer"}
                onBack={() => setStep(1)} onNext={() => { setViaWeek(false); setStep(3); }}
                onPickSlot={(d, s) => { setDate(d); setSlot(s); setViaWeek(true); setStep(4); }} />
            )}
            {step === 3 && service && (
              <StepSlots saloonId={String(saloon.id)} serviceId={service.id} date={date}
                staffId={staffId} staffMap={staffMap} selectedSlot={slot} accent={accent}
                onSelect={setSlot} onBack={() => setStep(2)} onNext={() => { setViaWeek(false); setStep(4); }} />
            )}
            {step === 4 && (
              <StepDetails form={form} setForm={setForm} accent={accent}
                onBack={() => setStep(viaWeek ? 2 : 3)} onSubmit={submitBooking} busy={busy} />
            )}
            {step === 5 && confirmed && service && (
              <StepConfirm booking={confirmed} service={service} staff={staffMap.get(confirmed.staffId)}
                saloon={saloon} accent={accent} onExit={onExit} />
            )}
          </div>
        </div>
      </main>
