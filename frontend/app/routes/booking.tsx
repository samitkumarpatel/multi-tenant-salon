...
import { SALOON_ADMIN_API, SALOON_API, apiFetch, resolveSaloonUUID } from "~/lib/api";
          ...
          <>
            ...
            {expanded && (
              <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm p-5 flex items-stretch">
                ...
                {/* Overlay grid — fills remaining height */}
...
function BookingSettingsPanel({ saloon, onSaved }: { saloon: { id: string | number; bookingAdvanceDays?: number }; onSaved: (days: number) => void }) {
  ...
  async function save() {
    setSaving(true);
    try {
      const res = await apiFetch(`${SALOON_ADMIN_API}/${saloon.id}`, {
        method: "PUT",
          ...
          {ADVANCE_OPTIONS.map((opt) => (
            <button
              ...
              className={`w-full flex items-center justify-between px-5 py-3.5 text-left transition-colors cursor-pointer ${
                days === opt.days ? "bg-matcha-50" : "hover:bg-slate-50"
              }`}
            >
              <div>
              ...
              <span className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
                days === opt.days ? "border-matcha-600 bg-matcha-600" : "border-slate-300"
              }`}>
...
export default function BookingPage() {
  ...
  // Fetch available slots for the selected reschedule date
  useEffect(() => {
    ...
    apiFetch<AvailableSlot[]>(`${SALOON_API}/${String(sid)}/slots?${params}`)
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                {!rsForm.appointmentDate ? (
                  ...
                  // Filter by staff; when "any", deduplicate by time (prefer available over booked)
                    ...
                    <div className="grid grid-cols-4 sm:grid-cols-5 gap-1.5 max-h-44 overflow-y-auto pr-1">
                      {displaySlots.map((s) => {
                        ...
                        return (
                          ...
                          >
                            {isBooked
                              ? <span className="line-through opacity-50">{fmt12(s.startTime)}</span>
                              : fmt12(s.startTime)}
