...
import type {
  Salon, Owner, Location, ContactInfo, OperatingHours, SalonFeature,
  Country, SalonManageContext,
} from "~/lib/types";
...
export default function SalonOverview() {
  ...
  const [nameDraft, setNameDraft]         = useState(salon.name);
  const [ownerDraft, setOwnerDraft]       = useState<Owner>(salon.owner ?? { name: "", email: "" });
  const [locDraft, setLocDraft]           = useState<Location>(salon.location ?? {});
  const [contactDraft, setContactDraft]   = useState<ContactInfo>(salon.contact ?? {});
  const [featuresDraft, setFeaturesDraft] = useState<SalonFeature[]>(salon.features ?? []);
  const [hoursDraft, setHoursDraft]       = useState<OperatingHours[]>(cloneHours(salon.operatingHours));

  function startEdit(section: Section) {
    setNameDraft(salon.name);
    setOwnerDraft(salon.owner ?? { name: "", email: "" });
    setLocDraft(salon.location ?? {});
    setContactDraft(salon.contact ?? {});
    setFeaturesDraft(salon.features ?? []);
    setHoursDraft(cloneHours(salon.operatingHours));
    setSaveErr(null);
    setEditing(section);
  }
  ...
  async function save(section: Section) {
    ...
    try {
      let updated: Salon;
      if (section === "owner") {
        updated = await apiFetch<Salon>(`${SUPER_ADMIN_API}/salons/${salon.id}/owner`, {
          method: "PUT",
          body: JSON.stringify({
        ...
        updated = await apiFetch<Salon>(`${ADMIN_API}/${salon.id}/features`, {
          method: "PUT",
          body: JSON.stringify(featuresDraft),
        });
      } else {
        updated = await apiFetch<Salon>(`${ADMIN_API}/${salon.id}`, {
          method: "PUT",
          body: JSON.stringify({
            name:           section === "name"     ? nameDraft.trim() : salon.name,
            location:       section === "location" ? locDraft         : salon.location,
            contact:        section === "contact"  ? contactDraft     : salon.contact,
            operatingHours: section === "hours"    ? hoursDraft       : salon.operatingHours,
            bookingAdvanceDays:          salon.bookingAdvanceDays,
            businessRegistrationId:      salon.businessRegistrationId,
            showBusinessId:              salon.showBusinessId,
            bookingRequiresConfirmation: salon.bookingRequiresConfirmation,
          }),
        });
      }
      setSalon(updated);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-stone-900">{salon.name}</h1>
          <p className="text-xs text-stone-400 font-mono mt-0.5">{salon.handler}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <EditableCard
          ...
          onSave={() => save("name")}
          display={
            <p className="text-sm font-semibold text-stone-800">{salon.name}</p>
          }
          form={
            <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <EditableCard
          ...
          display={
            <div className="space-y-2">
              {(salon.location?.address || salon.location?.city || salon.location?.country) ? (
                <div className="flex items-start gap-3">
                  <MapPin className="w-3.5 h-3.5 text-stone-400 mt-0.5 shrink-0" />
                  <div>
                    {salon.location?.address && <p className="text-sm text-stone-800">{salon.location.address}</p>}
                    <p className="text-sm text-stone-600">
                      {[salon.location?.zipCode, salon.location?.city, salon.location?.country].filter(Boolean).join(", ")}
                    </p>
                  </div>
                </div>
              ) : (
