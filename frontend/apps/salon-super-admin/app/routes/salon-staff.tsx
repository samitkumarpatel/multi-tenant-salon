...
import type { StaffMember, SalonManageContext } from "~/lib/types";

export async function clientLoader({ params }: ClientLoaderFunctionArgs) {
  const staff = await apiFetch<StaffMember[]>(`${ADMIN_API}/${params.salonId!}/staff`);
  return { staff };
}
...
export default function SalonStaff() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editTarget) {
        const updated = await apiFetch<StaffMember>(`${ADMIN_API}/${salon.id}/staff/${editTarget.id}`, { method:"PUT", body: JSON.stringify(body) });
        setStaff((p) => p.map((m) => (m.id === updated.id ? updated : m)));
      } else {
        const created = await apiFetch<StaffMember>(`${ADMIN_API}/${salon.id}/staff`, { method:"POST", body: JSON.stringify(body) });
  ...
  async function handleDelete(id: number) {
    setDeleting(id);
    try {
      await apiFetch(`${ADMIN_API}/${salon.id}/staff/${id}`, { method: "DELETE" });
          ...
          <div className="divide-y divide-stone-200">
            {staff.map((m) => (
              <div key={m.id} className="flex items-center gap-4 px-4 py-3">
                {m.avatarUrl ? (
                  <img src={m.avatarUrl} alt={m.name} className="w-9 h-9 rounded-full object-cover shrink-0 border border-matcha-200" />
                ) : (
