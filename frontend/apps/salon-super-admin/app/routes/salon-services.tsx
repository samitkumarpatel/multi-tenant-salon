...
import type { ServiceItem, StaffMember, Country, SalonManageContext } from "~/lib/types";
...
export default function SalonServices() {
  ...
  async function handleCreate() {
    ...
    try {
      const created = await apiFetch<ServiceItem>(`${ADMIN_API}/${salon.id}/services`, {
        method: "POST",
        body: JSON.stringify({
  ...
  async function handleUpdate() {
    ...
    setSaving(true); setErr(null);
    try {
      const updated = await apiFetch<ServiceItem>(`${ADMIN_API}/${salon.id}/services/${editTarget.id}`, {
        method: "PUT",
        body: JSON.stringify({
          name: form.name.trim(), description: form.description.trim() || null,
          price: parseFloat(form.price) || 0, currency: form.currency,
  ...
  async function handleDelete(id: number) {
    ...
    try {
      await apiFetch(`${ADMIN_API}/${salon.id}/services/${id}`, { method: "DELETE" });
