...
export default function SaloonPicker() {
  ...
  useEffect(() => {
    ...
    if (!session || session.saloons.length === 0) {
      navigate("/login", { replace: true });
      return;
    }
    setSaloons(session.saloons);
  ...
  if (saloons.length === 0) return null;
      ...
      <div className="flex flex-1 items-start justify-center px-4 pt-12 pb-10">
        <div className="w-full max-w-[480px]">
          ...
          <div className="mb-6 text-center">
            ...
            <h1 className="text-lg font-bold text-slate-900">Choose your saloon</h1>
            <p className="text-xs text-slate-500 mt-1">
              Multiple saloons are linked to <span className="font-medium text-slate-700">{email}</span>.
