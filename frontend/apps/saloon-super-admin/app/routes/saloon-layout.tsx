...
import type { Saloon, SaloonManageContext } from "~/lib/types";

export async function clientLoader({ params }: ClientLoaderFunctionArgs) {
  if (!getSession()) throw redirect("/login");
  const saloon = await apiFetch<Saloon>(`${ADMIN_API}/${params.saloonId}`);
  return { saloon };
}
...
export default function SaloonLayout() {
  const { saloon: initial } = useLoaderData<typeof clientLoader>();
  ...
  const ctx: SaloonManageContext = { saloon, setSaloon };
      ...
      <header className="h-12 border-b border-stone-200 bg-white/80 flex items-center px-4 gap-3 shrink-0 sticky top-0 z-30">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          >
            ...
            <span className="hidden sm:inline">All Saloons</span>
          </Link>
          <button
