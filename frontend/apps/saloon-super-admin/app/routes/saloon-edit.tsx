...
import type { Saloon, Owner, Location, ContactInfo, OperatingHours, Country, SaloonFeature, SaloonManageContext } from "~/lib/types";
...
const ALL_FEATURES: SaloonFeature[] = ["STATIC_WEBSITE", "BOOKING", "MEMBERSHIP", "WEBSHOP", "ANALYTICS", "LOYALTY_PROGRAM"];
...
const STEPS = [
  { title: "Saloon name",   hint: "Update the saloon's display name." },
  { title: "Ownership",     hint: "Change who owns and manages this saloon." },
  { title: "Location",      hint: "Where is the saloon located?" },
  { title: "Contact info",  hint: "How can customers reach the saloon?" },
  { title: "Features",      hint: "Enable or disable features for this saloon." },
...
export default function SaloonEdit() {
  ...
  const [owner, setOwner] = useState<Owner>(saloon.owner ? { ...saloon.owner } : { name: "", email: "", phone: "" });
  ...
  async function handleSave() {
    ...
    try {
      ...
      const ownerChanged =
        owner.name.trim()  !== (saloon.owner?.name  ?? "") ||
        owner.email.trim() !== (saloon.owner?.email ?? "") ||
        (owner.phone?.trim() || null) !== (saloon.owner?.phone ?? null);
      if (ownerChanged) {
        await apiFetch<Saloon>(`${SUPER_ADMIN_API}/saloons/${saloon.id}/owner`, {
          method: "PUT",
          body: JSON.stringify({
        ...
        return (
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="email" className={inp} value={contact.email ?? ""} onChange={(e) => patchCon({ email: e.target.value })} placeholder="hello@saloon.com" />
            </div>
            <div>
              <label className={lbl}>Website</label>
              <input className={inp} value={contact.website ?? ""} onChange={(e) => patchCon({ website: e.target.value })} placeholder="https://saloon.com" />
            </div>
          </div>
        );
      case 4:
  ...
  return (
    <div className="max-w-lg">
      <h1 className="text-xl font-bold text-stone-900 mb-6">Edit Saloon</h1>
