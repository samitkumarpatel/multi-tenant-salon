      ...
      <aside className="fixed right-0 top-0 h-full w-full max-w-md bg-white border-l border-stone-200 z-50 flex flex-col shadow-2xl overflow-y-auto">
        ...
        <div className="flex items-start gap-3 px-6 py-5 border-b border-stone-200">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-stone-900 truncate">{saloon.name}</p>
            <p className="text-[11px] text-stone-400 mt-0.5 font-mono truncate">{saloon.handler}</p>
          </div>
        ...
        <div className="flex-1 px-6 py-5 space-y-6">
          ...
          {isDisabled && (
            <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-red-500">Saloon is disabled</p>
            ...
            <dl className="space-y-2">
              {saloon.owner?.name && (
                <div className="flex items-center gap-3">
                  ...
                  <dd className="text-xs text-stone-600">{saloon.owner.name}</dd>
                </div>
              )}
              {saloon.owner?.email && (
                <div className="flex items-center gap-3">
                  ...
                  <dd className="text-xs text-stone-600 font-mono">{saloon.owner.email}</dd>
                </div>
              )}
              ...
              <div className="flex items-center gap-3">
                <dt className="text-xs text-stone-400 min-w-[80px]">Saloon ID</dt>
                <dd className="text-xs text-stone-400 font-mono truncate">{saloon.id}</dd>
              </div>
              <div className="flex items-center gap-3">
                <dt className="text-xs text-stone-400 min-w-[80px]">Created</dt>
                <dd className="text-xs text-stone-500">{formatDate(saloon.createdAt)}</dd>
              </div>
            </dl>
          </section>
              ...
              {!confirmDelete ? (
                ...
                <div className="border border-red-200 rounded-xl bg-red-50 p-4 space-y-3">
                  <p className="text-xs text-red-500 leading-relaxed">
                    This will disable <strong className="text-red-700">{saloon.name}</strong>. It will stop accepting bookings and be hidden publicly. All data is preserved and can be re-enabled.
                  </p>
...
export default function SuperAdminHome() {
  ...
  async function loadSaloons(q: string, status: "ALL" | "ACTIVE" | "DISABLED") {
    setLoading(true);
    setError(null);
    try {
      ...
      const data = await apiFetch<Saloon[]>(`${SUPER_ADMIN_API}/saloons${qs ? `?${qs}` : ""}`);
      setSaloons(data);
    } catch (e) {
  ...
  function handleSearchChange(value: string) {
    ...
    debounceRef.current = setTimeout(() => loadSaloons(value, statusFilter), 400);
  }

  function handleStatusChange(status: "ALL" | "ACTIVE" | "DISABLED") {
    setStatusFilter(status);
    if (debounceRef.current) clearTimeout(debounceRef.current);
    loadSaloons(search, status);
  }
  ...
  return (
    <div className="min-h-[100dvh] bg-stone-50 flex flex-col">
      ...
      {/* Top bar */}
      <header className="h-12 border-b border-stone-200 bg-white/80 flex items-center px-4 sm:px-6 gap-3 shrink-0 sticky top-0 z-30">
        ...
        <div className="ml-auto flex items-center gap-2">
          <button
            onClick={() => loadSaloons(search, statusFilter)}
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 py-6">
        ...
        {!loading && !error && (
          <div className="grid grid-cols-3 gap-3 mb-6">
            {[
              { label: "Total saloons", value: saloons.length, color: "text-stone-800" },
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 gap-3">
            ...
            <p className="text-sm text-stone-400">Loading saloons…</p>
          </div>
        ) : error ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-stone-200 text-left">
                  <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-stone-400">Saloon</th>
    ...
    >
      <div className="flex items-start gap-3 mb-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-stone-400 font-mono mt-0.5 truncate">{saloon.handler}</p>
        </div>
      </div>
      ...
      <div className="mt-4 pt-3 border-t border-stone-200 flex items-center justify-between">
        <span className="text-[11px] text-stone-400">{formatDate(saloon.createdAt)}</span>
