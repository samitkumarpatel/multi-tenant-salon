...
import type { SaloonHoliday, SaloonManageContext } from "~/lib/types";

const MONTH_NAMES = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"] as const;

function fmtHoliday(h: SaloonHoliday) {
...
export default function SaloonHolidays() {
  const { saloon } = useOutletContext<SaloonManageContext>();
  const sid = String(saloon.id);
  ...
  useEffect(() => {
    apiFetch<SaloonHoliday[]>(`${ADMIN_API}/${sid}/holidays`)
  ...
  async function addHoliday() {
    ...
    try {
      ...
      const saved = await apiFetch<SaloonHoliday>(`${ADMIN_API}/${sid}/holidays`, {
        method: "POST",
        body: JSON.stringify({
  ...
  function HolidayRow({ h }: { h: SaloonHoliday }) {
    return (
