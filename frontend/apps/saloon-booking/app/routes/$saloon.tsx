...
import { DEFAULT_THEME, BookingWizard, GenerativeUIBooking, SalonErrorPage, apiFetch, API_BASE, contrastText } from "@saloon/ui-website";
import type { Saloon, ServiceItem, StaffMember, WebsiteTheme, Country } from "@saloon/ui-website";
...
type LoaderData =
  | LoaderOk
  | { status: "booking_disabled"; saloonName?: string }
  | { status: "not_found" }
  | { status: "error" };
...
export async function clientLoader({ params }: ClientLoaderFunctionArgs): Promise<LoaderData> {
  const handle = params.saloon!;
  try {
    const saloon = await apiFetch<Saloon>(`${API_BASE}/api/saloon/${handle}`);

    if (saloon.status === "DISABLED" || !saloon.features?.includes("BOOKING")) {
      return { status: "booking_disabled", saloonName: saloon.name };
    }
    ...
    const [services, staff, theme, countries] = await Promise.all([
      ...
      apiFetch<Country[]>(`${API_BASE}/api/saloon-utility/countries`).catch((): Country[] => []),
    ]);
...
function BookingUnavailablePage({ saloonName }: { saloonName?: string }) {
  return (
    <div
    ...
    >
      ...
      <h1 className="text-2xl sm:text-3xl font-black text-white mb-3 leading-snug">
        {saloonName ? `${saloonName} isn't taking online bookings` : "Online booking unavailable"}
      </h1>
...
export default function SaloonBookingRoute() {
  ...
  useEffect(() => {
    ...
    document.title = `Book at ${data.saloon.name}`;
  ...
  if (data.status === "booking_disabled") return <BookingUnavailablePage saloonName={data.saloonName} />;

  const { saloon, services, staff, theme, countries } = data;
