...
export async function clientLoader({ params }: ClientLoaderFunctionArgs) {
  ...
  const orders = await apiFetch<ShopOrder[]>(`${ADMIN_API}/${sid}/shop/credit-notes`);
  return { sid, orders };
}
...
export default function ShopCreditNotes() {
  ...
  async function payCreditNote(order: ShopOrder) {
    setBusy(order.id);
    try {
      const updated = await apiFetch<ShopOrder>(
        `${ADMIN_API}/${sid}/shop/orders/${order.id}/credit-note/pay`,
        { method: "POST" },
      );
            ...
            {orders.map((o) => (
              <tr key={o.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-4 py-3 text-slate-400 text-xs">
                  {o.creditNoteAt ? relativeTime(o.creditNoteAt) : relativeTime(o.createdAt)}
                </td>
