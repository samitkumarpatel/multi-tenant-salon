...
export default function NewSalon() {
  ...
  async function handleCreate() {
    setSaving(true);
    try {
      const result = await apiFetch<{ id: string; handler: string }>(SALON_ONBOARDING_API, {
        method: "POST",
        body: JSON.stringify({
          name:       form.name.trim(),
  ...
  if (created) {
    return <SuccessScreen id={created.id} handler={created.handler} ownerEmail={form.owner.email} salonName={form.name} />;
  }
