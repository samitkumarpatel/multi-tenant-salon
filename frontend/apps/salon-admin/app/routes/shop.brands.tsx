...
export async function clientLoader({ params }: ClientLoaderFunctionArgs) {
  ...
  const brands = await apiFetch<ShopBrand[]>(`${ADMIN_API}/${sid}/shop/brands`);
  return { sid, brands };
}
...
export default function ShopBrands() {
  ...
  const [target, setTarget] = useState<ShopBrand | null>(null);
  ...
  function openEdit(b: ShopBrand) {
    setTarget(b);
    setF({ name: b.name, description: b.description ?? "", active: b.active });
    setModal({ kind: "edit" });
  }
  function openDel(b: ShopBrand) {
    setTarget(b);
    setModal({ kind: "del" });
  }
  ...
  async function submitAdd() {
    ...
    try {
      ...
      setBrands((p) => [...p, created].sort((a, b) => a.name.localeCompare(b.name)));
      close();
  ...
  async function submitEdit() {
    ...
    setBusy(true);
    try {
      const updated = await apiFetch<ShopBrand>(`${ADMIN_API}/${sid}/shop/brands/${target.id}`, {
        method: "PUT",
        body: JSON.stringify({ name: f.name.trim(), description: f.description || null, active: f.active }),
      });
      setBrands((p) => p.map((b) => (b.id === updated.id ? updated : b)));
      close();
  ...
  async function submitDel() {
    ...
    setBusy(true);
    try {
      await apiFetch(`${ADMIN_API}/${sid}/shop/brands/${target.id}`, { method: "DELETE" });
      const name = target.name;
      setBrands((p) => p.filter((b) => b.id !== target.id));
      close();
      notify(`"${name}" removed`);
    } catch (e) {
      notify(e instanceof Error ? e.message : "Error", "error");
