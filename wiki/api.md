...
```json
{
  ...
  "imageUrl": "http://localhost:8080/api/media/photos/uploads/product/12/uuid.jpg",
  "active": true,
  "variants": [
...
| `POST /products/{productId}/image-upload-url` | `{ contentType }` → `{ presignedUrl, publicUrl }` (PUT the file to `presignedUrl`, then save `publicUrl` as `imageUrl`) |
...
```json
{
  ...
  "brandId": 3, "categoryId": 1, "imageUrl": null, "active": true,
  "variants": [
...
| Method & path | Purpose |
|---|---|
| `GET /orders` | Newest first; line `activities` omitted |
| `GET /orders/{orderId}` | Full order incl. each line's **activity timeline** |
