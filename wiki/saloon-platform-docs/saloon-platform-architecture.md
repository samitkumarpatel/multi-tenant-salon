...
Companion diagrams (in `/diagrams`):
- `saloon-platform-module-overview.svg` — domain routing → four micro-frontends → shared backend API
- `shared-website-template-flow.svg` — how `saloon-admin` design mode and `saloon-public-website` both render the same template module against the same public API
